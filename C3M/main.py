...
parser.add_argument('--dim', type=str, default='xy', help='Time constant of accel.')
...
def forward(x, xref, uref, _lambda, verbose=False, acc=False, detach=False):
    ...
    if verbose:
        print(torch.linalg.eigh(Contraction)[0].min(dim=1)[0].mean(), torch.linalg.eigh(Contraction)[0].max(dim=1)[0].mean(), torch.linalg.eigh(Contraction)[0].mean())
    if acc:
        return loss, ((torch.linalg.eigh(Contraction)[0]>=0).sum(dim=1)==0).cpu().detach().numpy(), ((torch.linalg.eigh(C1_LHS_1)[0]>=0).sum(dim=1)==0).cpu().detach().numpy(), sum([1.*(C2**2).reshape(bs,-1).sum(dim=1).mean() for C2 in C2s]).item()
    else:
