...
void Station::_connectVicon()
{
	...
	vicon_.SetAxisMapping(Direction::Right,Direction::Forward,Direction::Up);
...
void Station::initialize()
{
    ...
    hostAddress = QHostAddress("192.168.1.2");
    hostPort = 801;
    north = "-x";
}
...
{
    isInitialized = false;
    viconConnectFuture = QtConcurrent::run(&Station::_connectVicon, this);
}
...
void Station::addDrone(const QString &name)
{
    ...
    droneCollection.insert(name, Drone(name, 1. / rate));
...
{
    dataStreamFuture = QtConcurrent::run(&Station::_dataStream, this);
}
...
DroneState Station::getState(const QString& name, uint64_t &time_boot_us)
{
    lock.lockForRead();
    DroneState ret = droneCollection[name].getState(time_boot_us);
    lock.unlock();
    return ret;
}
