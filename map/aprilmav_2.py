    ...
    def sendPos(self):
        ...
        if self.goodToSend:
            ...
            covariance = numpy.array([cov_pose, 0, 0, 0, 0, 0,
                                      ...
                                      cov_twist])
            with self.lock:
                # print(-self.pos[1], self.pos[0], self.pos[2])
                
                self.conn.mav.att_pos_mocap_send(current_time_us, self.rot, self.pos[1], self.pos[0], -self.pos[2])
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # print("ROT:", numpy.round(numpy.array(euler_from_quaternion(rotD, 'sxyz')) / numpy.pi  * 180.),2)
        ...
        # current_time_us = int(round(time.time() * 1000000))
        # threadMavlink.conn.mav.att_pos_mocap_send(current_time_us, rotD, posD[1], posD[0], -posD[2])
        
        # print("POSD",numpy.round(posR,2))
        ...
        ) - myStart, (posD[0], posD[1], posD[2]), (rotD[0], rotD[1], rotD[2], rotD[3]), threadMavlink.getPktSent())
